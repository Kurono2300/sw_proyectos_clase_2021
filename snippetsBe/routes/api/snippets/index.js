...
const {getAll, getAllAsync, getById, getByIdAsync, addNew, addNewAsync, 
  update, updateAsync, deleteRow, deleteAsync } = require("./snippets.model");
...
router.get("/byidasync/:id", async (req, res)=>{
    const { id } = req.params;
    try {
...
router.post("/newasync", async (req, res)=>{
    const { snippetName, snippet, stated } = req.body;
    //validaciones
    try{
...
router.put("/updateasync/:id", async (req, res)=>{
    const { id } = req.params;
    const { snippetName, snippet, stated } = req.body;
    //validaciones
    try {
